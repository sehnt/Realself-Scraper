...
def parse_reviews(procedure):
    ...
    for link in links_file.readlines():
        link = link.strip()
        review_path = (directory / "webpages") / (unique_id(link) + ".txt")
        
        if len(temp_group) < PARSE_THREAD_COUNT:
    ...
    for group in thread_groups:
        pool = ThreadPool(PARSE_THREAD_COUNT)
        pool.map(parse_review, group)
        #print(failed_links)
...
def visit_links(procedure, delay):
